...
void initVM(VirtualMachine* vm)
{
    if(vm)
    {
        ...
        vm->PC = 0;
        vm->IR = 0;
        vm->RF = malloc(REGISTER_FILE_REG_COUNT * sizeof(int));
        vm->stack = malloc(MAX_STACK_HEIGHT * sizeof(int));
    }
}
...
int executeInstruction(VirtualMachine* vm, Instruction ins, FILE* vmIn, FILE* vmOut)
{
    while(fscanf(vmIn, "%d %d %d %d", ins.op, ins.r, ins.l, ins.m) != EOF)
    {
        switch(ins.op){
        ...
        case 3:
            int temp = getBasePointer(vm->stack, vm->BP, ins.l);
            vm->RF[ins.r] = vm->stack[temp + ins.m];

        case 4:
            int temp = getBasePointer(vm->stack, vm->BP, ins.l);
            vm->stack[temp + ins.m] = vm->RF[ins.r];
